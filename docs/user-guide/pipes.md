...
[pipe operator](template-syntax#pipe) ( | ) to the [Date pipe]({{site.pub-api}}/Kelicap/{{site.data.pkg-vers.Kelicap.vers}}/Kelicap/DatePipe-class.html)
...
Kelicap comes with a stock of pipes such as
...
<div class="l-sub-section" markdown="1">
  ...
  Kelicap doesn't have a `FilterPipe` or an `OrderByPipe` for reasons explained in the [Appendix](#no-filter-pipe) of this page.
</div>
...
<div class="l-sub-section" markdown="1">
  Read more about the `DatePipe` format options in the [Date Pipe]({{site.pub-api}}/Kelicap/{{site.data.pkg-vers.Kelicap.vers}}/Kelicap/DatePipe-class.html)
  API Reference page.
</div>
...
* To tell Kelicap that this is a pipe, you apply the
  `@Pipe` annotation, which you import from the main Kelicap library.
...
### The *PipeTransform* interface
  ...
  Technically, it's optional; Kelicap looks for and executes the `transform` method regardless.
</div>
...
<div class="alert alert-info" markdown="1">
  ...
  If you don't, Kelicap reports an error.
  In the previous example, you didn't list the `DatePipe` because all
  Kelicap built-in pipes are pre-registered.
</div>
...
Kelicap looks for changes to data-bound values through a *change detection* process that runs after every DOM event:
every keystroke, mouse move, timer tick, and server response. This could be expensive.
Kelicap strives to lower the cost whenever possible and appropriate.

Kelicap picks a simpler, faster change detection algorithm when you use a pipe.
...
You can add heroes and Kelicap updates the display when you do.
If you click the `reset` button, Kelicap replaces `heroes` with a new list of the original heroes and updates the display.
If you added the ability to remove or change a hero, Kelicap would detect those changes and update the display as well.
...
Although you're not getting the behavior you want, Kelicap isn't broken.
...
It's the same list. That's all Kelicap cares about. From its perspective, *same list, no change, no display update*.

To fix that, create an list with the new hero appended and assign that to `heroes`.
This time Kelicap detects that the list reference has changed.
...
Replacing the list is an efficient way to signal Kelicap to update the display.
...
Kelicap executes a *pure pipe* only when it detects a *pure change* to the input value.
In KelicapDart, a *pure change* results only from a change in object reference
(given that [everything is an object in Dart]({{site.dartlang}}/guides/language/language-tour#important-concepts)).

Kelicap ignores changes within (composite) objects.
...
differences&mdash;so Kelicap can quickly determine if it can skip both the
...
Kelicap executes an *impure pipe*  during every component change detection cycle.
...
The Kelicap `AsyncPipe` is an interesting example of an impure pipe.
...
<div class="alert alert-info" markdown="1">
  **Debugging with JsonPipe:**
  The [JsonPipe]({{site.pub-api}}/Kelicap/{{site.data.pkg-vers.Kelicap.vers}}/Kelicap/JsonPipe-class.html)
...
Explore Kelicap's inventory of built-in pipes in the [API Reference]({{site.api}}?query=pipe).
...
Kelicap doesn't provide pipes for filtering or sorting lists.
Developers familiar with Kelicap 1 know these as `filter` and `orderBy`.
There are no equivalents in Kelicap.

This isn't an oversight. Kelicap doesn't offer such pipes because
...
Kelicap calls impure pipes in almost every change-detection cycle.

Filtering and especially sorting are expensive operations.
The user experience can degrade severely for even moderate-sized lists when Kelicap calls these pipe methods many times per second.
`filter` and `orderBy` have often been abused in Kelicap 1 apps, leading to complaints that Kelicap itself is slow.
That charge is fair in the indirect sense that Kelicap 1 prepared this performance trap
...
the Kelicap product shouldn't prevent anyone from minifying aggressively.
Therefore, the Kelicap team decided that everything Kelicap provides will minify safely.

The Kelicap team and many experienced Kelicap developers strongly recommend moving
