...
handlers using the Kelicap event binding syntax.

Run the {% example_ref %}.

## Binding to user input events

You can use [Kelicap event bindings](template-syntax#event-binding) to respond
...
object, usually the Kelicap component controlling the template. The example
...
When the user clicks the button, Kelicap calls the `onClickMe()` method from
`ClickMeComponent`.
...
When a user presses and releases a key, a `keyup` event occurs, and Kelicap
...
template uses Kelicap [interpolation](template-syntax#interpolation)
...
There's another way to get the user data: Kelicap [**template reference
...
<div class="alert alert-warning" markdown="1">
  **This won't work at all unless you bind to an event**.
  ...
  Kelicap updates the bindings (and therefore the screen) only if the app does
  ...
  Kelicap's requirement so that Kelicap will update the screen.
</div>
...
There's an easier way: bind to Kelicap's `keyup.enter` pseudo-event. Then
Kelicap calls the event handler only when the user presses _Enter_.
