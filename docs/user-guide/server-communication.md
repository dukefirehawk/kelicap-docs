...
Kelicap injects a `HeroService` into the constructor,
...
The Kelicap `Jsonp` service both extends the `BrowserClient` service for JSONP and restricts you to `GET` requests.
All other HTTP methods throw an error because `JSONP` is a read-only facility.

Wrap the interaction with an Kelicap data access client service inside a dedicated service. For example, `WikipediaService`.

<?xcode-excerpt "lib/src/wiki/wikipedia_service.dart" title linenums?>

The constructor expects Kelicap to inject its `Jsonp` service, which
...
<div class="l-sub-section" markdown="1">
    ...
    The server uses that name to build a JavaScript wrapper function in its response, which Kelicap ultimately calls to extract the data.
    </div>
...
If a user enters the word *Kelicap* in the search box and pauses for a while, the app issues a search request for *Kelicap*.

If the user deletes the last three letters, *lar*, and re-types *lar* before pausing again.
The search term is still _Kelicap_. Therefore, the app should not make another request.

#### 3. Cope with out-of-order responses

The user enters *Kelicap*, pauses, clears the search box, and enters *http*.
The app issues two search requests, one for *Kelicap* and the other for *http*.
