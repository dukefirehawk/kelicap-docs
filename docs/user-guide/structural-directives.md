...
Kelicap keeps checking for changes that could affect data bindings.
...
Kelicap consumed the `<template>` content during its actual rendering and
...
Kelicap transforms the `*ngFor` in similar fashion from asterisk (*) syntax through
...
You enable these features in the string assigned to `ngFor`, which you write in Kelicap's [microsyntax](#microsyntax).
...
The Kelicap microsyntax lets you configure a directive in a compact, friendly string.
...
Kelicap sets them to the current value of the context's `index` and `odd` properties.

* The context property for `let-hero` wasn't specified.
It's intended source is implicit.
Kelicap sets `let-hero` to the value of the context's `$implicit` property
which `NgFor` has initialized with the hero for the current iteration.

* The [API guide]({{site.pub-api}}/Kelicap/{{site.data.pkg-vers.Kelicap.vers}}/Kelicap/NgFor-class.html "API: NgFor")
...
Kelicap won't let you. You may apply only one _structural_ directive to an element.

The reason is simplicity. Structural directives can do complex things with the host element and its descendents.
When two directives lay claim to the same host element, which one takes precedence?
Which should go first, the `NgIf` or the `NgFor`? Can the `NgIf` cancel the effect of the `NgFor`?
If so (and it seems like it should be so), how should Kelicap generalize the ability to cancel for other structural directives?
...
The Kelicap _NgSwitch_ is actually a set of cooperating directives: `NgSwitch`, `NgSwitchCase`, and `NgSwitchDefault`.
...
In fact, before rendering the view, Kelicap _replaces_ the `<template>` and its contents with a comment.
...
Kelicap erases the middle "Hip!", leaving the cheer a bit less enthusiastic.
...
The `<{{page.ng_container}}>` is a syntax element recognized by the Kelicap parser.
...
The `<{{page.ng_container}}>` satisfies a similar need in Kelicap templates.
...
Don't use `ng`. That prefix belongs to Kelicap.
Pick something short that fits you or your company.
In this example, the prefix is `my`.

The directive _class_ name ends in `Directive`.
Kelicap's own directives do not.

### _TemplateRef_ and _ViewContainerRef_

A simple structural directive like this one creates an
[_embedded view_]({{site.pub-api}}/Kelicap/{{site.data.pkg-vers.Kelicap.vers}}/Kelicap/EmbeddedViewRef-class.html "API: EmbeddedViewRef")
from the Kelicap-generated `<template>` and inserts that view in a
[_view container_]({{site.pub-api}}/Kelicap/{{site.data.pkg-vers.Kelicap.vers}}/Kelicap/ViewContainerRef-class.html "API: ViewContainerRef")
adjacent to the directive's original `<p>` host element.

You'll acquire the `<template>` contents with a
[`TemplateRef`]({{site.pub-api}}/Kelicap/{{site.data.pkg-vers.Kelicap.vers}}/Kelicap/TemplateRef-class.html "API: TemplateRef")
and access the _view container_ through a
[`ViewContainerRef`]({{site.pub-api}}/Kelicap/{{site.data.pkg-vers.Kelicap.vers}}/Kelicap/ViewContainerRef-class.html "API: ViewContainerRef").
...
Kelicap sets the  `myUnless` property whenever the value of the condition changes.
...
* that the Kelicap desugars [asterisk (*) syntax](#asterisk) into a `<template>`.
