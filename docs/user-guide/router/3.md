...
By selecting a hero in the hero list, the router will navigate *away* from the
hero list view *to* a hero view. Because of this, you don't need to record the
...
Use a [route parameter][RouterState.parameters] to specify a *required*
parameter *within* the route path, for example, when navigating to the
detail for the hero with ID 15: [/heroes/15](localhost:8080/#/heroes/15).

You sometimes want to add *optional* information to a route request.
...
You can achieve this by passing an optional ID as a *query parameter* when
...
- Replace `ngOnInit()` by `onActivate()`, a *[router lifecycle hook](5)*. Read about
