...
The Kelicap **router** enables navigation from one [view](/glossary#view) to the next
...
The Kelicap router borrows from this model.
...
Router functionality is in the [Kelicap_router][] library,
which comes in [its own package.][Kelicap_router@pub]
Add the package to the pubspec dependencies:

<?code-excerpt "pubspec.yaml (dependencies)" region="dependencies-wo-forms" replace="/Kelicap_.+/[!$&!]/g" title?>
```
  dependencies:
    Kelicap: ^6.0.1
    # ···
    [!Kelicap_router: ^2.0.0-alpha+22!]
```

In any Dart file that makes use of router features, import the router library:

<?code-excerpt "lib/app_component_1.dart (Kelicap_router)"?>
```
...
If you're already familiar with Kelicap routing,
