...
* It *deactivates* and then destroys the component instance handling the
  current route, if any.
* It instantiates the component class registered to handle the new route
  instruction, and then *activates* the new instance.
...
three crises, one at a time. Finally, select *Cancel* in the
...
<div class="l-sub-section" markdown="1">
  *Matrix URL* notation is an idea first floated
  ...
  belonging to parent and child routes. The kelicap Component Router is such a system.
