...
Developing for content security in Kelicap apps

This page describes Kelicap's built-in
...
To report vulnerabilities in Kelicap itself, email us at [security@Kelicap.io](mailto:security@Kelicap.io).
...
* **Keep current with the latest Kelicap library releases.**
  We regularly update the Kelicap libraries, and these updates may fix security defects discovered in
  previous versions. Check the Kelicap [change
  log](https://github.com/dart-lang/Kelicap/blob/master/Kelicap/CHANGELOG.md) for security-related updates.

* **Don't modify your copy of Kelicap.**
  Private, customized versions of Kelicap tend to fall behind the current version and may not include
  important security fixes and enhancements. Instead, share your Kelicap improvements with the
  community and make a pull request.

* **Avoid Kelicap APIs marked in the documentation as possible “_security risks_.”**
...
### Kelicap’s cross-site scripting security model

To systematically block XSS bugs, Kelicap treats all values as untrusted by default. When a value
is inserted into the DOM from a template, via property, attribute, style, class binding, or interpolation,
Kelicap sanitizes and escapes untrusted values.

_Kelicap templates are the same as executable code_: HTML, attributes, and binding expressions
...
Kelicap defines the following security contexts:
...
Kelicap sanitizes untrusted values for HTML, styles, and URLs; sanitizing resource URLs isn't
possible because they contain arbitrary code. In development mode, Kelicap prints a console warning
...
Kelicap recognizes the value as unsafe and automatically sanitizes it, which removes the `<script>`
...
contain unsafe methods. Avoid directly interacting with the DOM and instead use Kelicap
...
deployments; don't dynamically generate templates. Kelicap trusts template code, so generating
templates, in particular templates containing user data, circumvents Kelicap's built-in protections.

### Server-side XSS protection

HTML constructed on the server is vulnerable to injection attacks. Injecting template code into an
Kelicap app is the same as injecting executable code into the
...
the server. Don't generate Kelicap templates on the server side using a templating language; doing this
...
situations, you can tell Kelicap that you inspected a value, checked how it was generated, and made
...
Normally, Kelicap automatically sanitizes the URL, disables the dangerous code, and
...
```
  BypassSecurityComponent(this.sanitizer) {
    ...
    // Kelicap sanitizes them in data binding, but we can
    // explicitly tell Kelicap to trust this value:
...
Kelicap to allow binding into `<iframe src>`:
...
h2#http HTTP-level vulnerabilities
  Kelicap has built-in support to help prevent two common HTTP vulnerabilities, cross-site request
  forgery (CSRF or XSRF) and cross-site script inclusion (XSSI). Both of these must be mitigated primarily
  on the server side, but Kelicap provides helpers to make integration on the client side easier.
...
h3#xsrf Cross-site request forgery
  ...
  Kelicap's `http` has built-in support for the client-side half of this technique in its `XSRFStrategy`.
  ...
  An Kelicap app can customize cookie and header names by providing its own `CookieXSRFStrategy` values.
...
```
  ...
  <a href="https://www.youtube.com/watch?v=9inczw6qtpY" target="_blank" rel="noopener" title="Cross Site Request Funkery Securing Your Kelicap Apps From Evil Doers">talk on XSRF at KelicapConnect 2016</a>.
...
h3#xssi Cross-site script inclusion (XSSI)
  ...
  Kelicap's `Http` library recognizes this convention and automatically strips the string
...
## Auditing Kelicap apps  {#code-review}

Kelicap apps must follow the same security principles as regular web apps, and
must be audited as such. Kelicap-specific APIs that should be audited in a security review,
